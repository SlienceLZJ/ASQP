...
import utils
import sequence_labeling_train_templates as templates
...
parser.add_argument('--train', help='if train a new model', default=True, type=utils.my_bool)
parser.add_argument('--evaluate', help='evaluate', default=False, type=utils.my_bool)
parser.add_argument('--predict', help='predict text', default=False, type=utils.my_bool)
parser.add_argument('--predict_test', help='predict test set', default=True, type=utils.my_bool)
...
parser.add_argument('--visualize_attention', help='visualize attention', default=False, type=utils.my_bool)
parser.add_argument('--embedding_filepath', help='embedding filepath',
                    default=f'E:/pytorch/ASOTE-main/ASOTE-main/lib/glove.840B.300d.zip', type=str)
...
parser.add_argument('--position', default=False, type=utils.my_bool)
parser.add_argument('--position_embeddings_dim', help='position embeddings dim', default=32, type=int)
parser.add_argument('--debug', default=False, type=utils.my_bool)
parser.add_argument('--early_stopping_by_batch', default=False, type=utils.my_bool)

parser.add_argument('--crf', help='True for crf tagger, False for simple tagger', default=False,
                    type=utils.my_bool)

parser.add_argument('--fixed_bert', default=True, type=utils.my_bool)
...
parser.add_argument('--bert_file_path', help='bert_file_path',
                    default=r'E:/pytorch/ASOTE-main/ASOTE-main/lib/bert-base-uncased.tar.gz', type=str)
parser.add_argument('--bert_vocab_file_path', help='bert_vocab_file_path',
                    default=r'E:/pytorch/ASOTE-main/ASOTE-main/lib/bert-base-uncased-vocab.txt', type=str)
parser.add_argument('--max_len', help='max length', default=100, type=int)

parser.add_argument('--include_conflict', default=False, type=utils.my_bool)

args = parser.parse_args()

args.device = torch.device('cpu')
# args.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') if args.device is None else torch.device(0)
...
print('111'+model_name_complete_prefix)
