    ...
    with open(data_path, 'r', encoding='UTF-8') as fp:
        ...
        for line in fp:
            ...
            if line != '':
                ...
                # print("words:{}".format(words.split()))
                labels.append(eval(tuples))
                # print("tuples:{}".format(tuples))
                # print("sents:{}".format(sents))
                # print("labels:{}".format(eval(tuples)))
    if silence:
...
def get_transformed_io(data_path, data_dir):
    ...
    """
    sents, labels = read_line_examples_from_file(data_path, 1)
...
class ABSADataset(Dataset):
    def __init__(self, tokenizer, data_dir, data_type, max_len=128):
        ...
        print(f'data/{data_dir}/{data_type}.txt')
    ...
    def __getitem__(self, index):
        ...
        # print("source_mask:{}".format(src_mask))
    ...
    def _build_examples(self):
        ...
        for i in range(len(inputs)):
            ...
            tokenized_input = self.tokenizer.batch_encode_plus(  # 通过预训练模型（T5）将input（句子）转换成向量的形式
